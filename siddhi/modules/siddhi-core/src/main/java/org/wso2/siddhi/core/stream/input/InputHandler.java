...
public class InputHandler {
    ...
    public void send(Object[] data) throws InterruptedException, IllegalAccessException, InstantiationException, InvocationTargetException, IOException {
        if (inputProcessor != null) {
            inputProcessor.send(System.currentTimeMillis(), data, streamIndex);
        }
    }

    public void send(long timestamp, Object[] data) throws InterruptedException, IllegalAccessException, InstantiationException, InvocationTargetException, IOException {
        if (inputProcessor != null) {
            inputProcessor.send(timestamp, data, streamIndex);
        }
    }

    public void send(Event event) throws InterruptedException, IllegalAccessException, InstantiationException, InvocationTargetException {
        if (inputProcessor != null) {
            inputProcessor.send(event, streamIndex);
        }
    }

    public void send(Event[] events) throws InterruptedException, IllegalAccessException, InstantiationException, InvocationTargetException {
        if (inputProcessor != null) {
